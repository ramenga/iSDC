...
def lowest_f(fScores,frontier,M,cameFrom,start):
    ...
    for node in frontier: #iterate through each node in frontier
        neighbors = set(M.roads[node])
        print("node:",node)
        #print("nodeneigh",neighbors)
        back = neighbors.intersection(camefrom_set)
        print("back",back)
        #research the most efficient path back to start for each frontier element
        costs=[]
        back_list=[]
        for each in back:
            back_list.append(each)
            path = reconstruct_path(cameFrom,each)
            if path[0] == start : #no compare to previous method, just search for it
                print("v",each)
                total=0.0
                for i in range(len(path)-1):
                    total += euclid_dist(M.intersections[path[i]],M.intersections[path[i+1]])
                costs.append(total)
        if len(costs)==0:
            continue
        lowest_back = [costs[0],0]
        for j in range(len(costs)):
            if costs[j]<lowest_back[0]:
                lowest_back[0]=costs[j]
                lowest_back[1]=j
        print("best backnode",back_list[lowest_back[1]])
